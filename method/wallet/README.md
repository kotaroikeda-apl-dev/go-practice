...
- **Account（ルールを持つ）**: 残高に関する全てのルール（バリデーション）を持つ
- **Wallet（調整役）**: Account と Transaction を調整し、履歴管理を担当
...
- **Account**: 残高に関する全てのルール（バリデーション）を持つ
- **Wallet**: Account と Transaction の調整役（ルールは Account に委譲、履歴管理を担当）
...
Account と Transaction をまとめて操作する構造体。調整役として機能する。

- `NewWallet(initialBalance int)`: 新しい Wallet を作成
- `Deposit(amount int)`: 入金を行い、取引履歴に追加（Account にルール適用を委譲）
- `Withdraw(amount int)`: 出金を行い、取引履歴に追加（Account にルール適用を委譲）
...
2. **責務の分離**: Account がルールを持ち、Wallet が調整役として機能する設計
